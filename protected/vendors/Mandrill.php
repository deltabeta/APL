...
abstract class Mandrill {
    ...
    private static function _validate_call(&$call_type, &$call, &$data) {
        ...
        if (!array_key_exists($call_type, $api_calls))
            throw new Exception('Invalid call type.');
        if (!array_key_exists($call, $api_calls[$call_type]))
            throw new Exception("Invalid call for call type $call_type");

        $diff_keys = array_diff(array_keys($data), $api_calls[$call_type][$call]);

        if (self::$verbose)
            error_log('MANDRILL: Invalid keys in call: ' . implode(',', $diff_keys));
    ...
    private static function _set_api_key(&$data) {
        ...
        if ((count($data) == 0 || is_null(self::$api_key)) && defined('MANDRILL_API_KEY'))
    ...
    private static function _call_api(&$data) {
        if (!array_key_exists('type', $data))
            throw new Exception('API call type must be set.');
        if (!array_key_exists('call', $data))
            throw new Exception('API call must be set.');
        ...
        if (!self::_validate_call($call_type, $call, $data))
            throw new Exception(self::$last_error);
       ...
       curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        ...
        if (self::_is_cli())
            echo "Mandrill API result: $result" . PHP_EOL;
        ...
        switch ($method) {
            ...
            case 'call':
                if (count($args) != 1 || !is_array($args[0]))
                    throw new Exception('Must pass one associative array with proper values set.');
                $args = $args[0];
